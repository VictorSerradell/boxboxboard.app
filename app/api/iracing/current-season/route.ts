...
export async function GET(request: NextRequest) {
  ...
  if (accessToken) {
    try {
      const res = await fetch(`${IRACING_BASE}/data/series/seasons?include_series=false`, {
        headers: { Authorization: `Bearer ${accessToken}`, 'User-Agent': 'BoxBoxBoard/1.0' },
      });
      if (res.ok) {
        const raw = await res.json();
        const data = raw?.link ? await fetch(raw.link).then(r => r.json()) : raw;
        const seasons: any[] = Array.isArray(data) ? data : data?.seasons ?? [];
        // Pick the season with highest year+quarter (most recent)
        const active = seasons
          .filter((s: any) => s.season_year && s.season_quarter)
          .sort((a: any, b: any) =>
            b.season_year !== a.season_year
              ? b.season_year - a.season_year
              : b.season_quarter - a.season_quarter
          )[0];
        if (active) {
