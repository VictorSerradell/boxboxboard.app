...
export async function GET(request: NextRequest) {
  ...
  if (!token) return NextResponse.json({ cars: [], total_drivers: 0, subsessions_sampled: 0 });
  ...
  try {
    ...
    console.log('[series-cars] season_id:', seasonId, 'week:', weekNum, 'sessions:', sessions.length);
    if (sessions.length > 0) console.log('[series-cars] session[0] keys:', Object.keys(sessions[0]).join(','));
    ...
    const topSessions = [...sessions]
      .sort((a, b) => (b.event_strength_of_field ?? 0) - (a.event_strength_of_field ?? 0))
      .slice(0, 8);
    ...
    const results = await Promise.allSettled(
      topSessions.map(s => iracingGet(`results/get?subsession_id=${s.subsession_id}`, token))
    );

    for (const r of results) {
      if (r.status !== 'fulfilled' || !r.value) continue;
      const raceSessions = (r.value?.session_results ?? [])
      ...
      for (const session of raceSessions) {
        for (const driver of (session.results ?? [])) {
          ...
          if (!carMap[carId]) carMap[carId] = { car_name: driver.car_name ?? `Car ${carId}`, best_laps: [] };
    ...
    cars.forEach((c, i) => { c.delta = i === 0 ? null : `+${((c.avg_lap_ms - leaderMs) / 10000).toFixed(3)}s`; });

    return NextResponse.json({ cars, total_drivers: totalDrivers, subsessions_sampled: topSessions.length });
