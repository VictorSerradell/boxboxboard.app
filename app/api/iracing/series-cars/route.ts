...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let subsessionIds: number[] = spectator?.subsession_ids ?? spectator?.session_ids ?? [];
    console.log('[series-cars] spectator subsessions for season', seasonId, ':', subsessionIds.length);
    ...
    const driverBests: { name: string; car_name: string; car_id: number; lap: number; irating: number; country: string }[] = [];
    ...
    for (const r of fetched) {
      ...
      if (data.race_week_num != null && Math.abs(data.race_week_num - weekNum) > 1) continue;

      // Prefer qualifying, fallback to race
      let session = (data.session_results ?? []).find((s: any) => s.simsession_type === 3);
      if (!session) session = (data.session_results ?? []).find((s: any) => s.simsession_type === 6);
    ...
    console.log('[series-cars] top car:', fastestCar, '| total drivers:', driverBests.length);
