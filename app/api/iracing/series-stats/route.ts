...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const week of [weekUsed, weekUsed - 1]) {
      ...
      console.log('[series-stats] step1 keys:', Object.keys(json1 ?? {}).join(','));

      let data: any[] = [];
      if (json1?.link) {
        const res2 = await fetch(json1.link, { signal: AbortSignal.timeout(12000) });
        console.log('[series-stats] step2 status:', res2.status);
        if (res2.ok) {
          const json2 = await res2.json();
          data = Array.isArray(json2) ? json2 : (json2?.results ?? json2?.sessions ?? []);
        }
      } else {
        data = Array.isArray(json1) ? json1 : (json1?.results ?? []);
      }

      console.log('[series-stats] week', week, 'sessions:', data.length, data[0] ? '| keys:' + Object.keys(data[0]).slice(0,6).join(',') : '');
      if (data.length > 0) {
        sessions = data;
        weekUsed = week;
        break;
...
function buildResponse(sessions: any[]) {
  ...
  const totalSof     = sessions.reduce((s: number, r: any) => s + (r.event_strength_of_field ?? 0), 0);
  const totalDrivers = sessions.reduce((s: number, r: any) => s + (r.num_drivers ?? 0), 0);
