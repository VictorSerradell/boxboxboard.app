...
import { Info, Coffee, Heart, LayoutGrid, LayoutList, CalendarClock, Github, ExternalLink, ChevronDown, User, Menu, X as XIcon } from 'lucide-react';
...
import { getSeasonList, getSeriesSeasons, getFavoriteSeriesIds } from '../lib/iracing-client';
...
export default function HomePage() {
  ...
  const [scheduledIds, setScheduledIds] = useState<number[]>(() => {
    try { return JSON.parse(localStorage.getItem('pitboard_schedule') ?? '[]'); } catch { return []; }
  });
  ...
  function toggleSchedule(id: number) {
    setScheduledIds(prev => {
      ...
      try { localStorage.setItem('pitboard_schedule', JSON.stringify(next)); } catch {}
      return next;
    });
  }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', color: 'var(--text-primary)', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{
        ...
        <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', flexShrink: 0 }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: 'linear-gradient(135deg, #3B9EFF 0%, #A855F7 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 13, color: 'white', boxShadow: '0 0 20px rgba(59,158,255,0.3)' }}>PB</div>
          {!isMobile && <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 20, color: 'var(--text-primary)', letterSpacing: '-0.5px' }}>PitBoard</span>}
        </a>
      ...
      {!user && (
        <div style={{
          ...
          <span style={{ fontSize: 13, color: '#64748B' }}>
            Showing <strong style={{ color: '#3B9EFF' }}>demo data</strong> — connect your iRacing account for live schedules and owned content.
          </span>
          <button
            ...
            <div style={{ display: 'flex', gap: 48, flexWrap: 'wrap' }}>
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {[
                    { label: 'GitHub', href: 'https://github.com/VictorSerradell/simplan.app', icon: <Github size={13} /> },
                    { label: 'Ko-fi', href: 'https://ko-fi.com', icon: <Coffee size={13} /> },
                  ].map(link => (
