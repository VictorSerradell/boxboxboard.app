          ...
          <nav style={{ display: 'flex', gap: 4, marginRight: 12 }}>
            ...
            {user && (
              <button onClick={() => setDriverProfileOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 14px', borderRadius: 10, border: 'none', cursor: 'pointer', fontFamily: 'Rajdhani, sans-serif', fontWeight: 600, fontSize: 13, background: 'transparent', color: '#64748B', transition: 'all 0.15s' }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#3B9EFF'; (e.currentTarget as HTMLElement).style.background = 'rgba(59,158,255,0.08)'; }}
        ...
        {!isMobile && (
          <>
            ...
            {user ? (
              ...
              <button onClick={() => setShowLogin(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 18px', borderRadius: 10, border: 'none', cursor: 'pointer', fontFamily: 'Rajdhani, sans-serif', fontWeight: 700, fontSize: 13, background: 'linear-gradient(135deg, #3B9EFF, #2563EB)', color: 'white', boxShadow: '0 0 20px rgba(59,158,255,0.25)' }}>
      ...
      {isMobile && menuOpen && (
        <div style={{ position: 'fixed', top: 64, left: 0, right: 0, bottom: 0, zIndex: 999, background: 'var(--bg-base)', overflowY: 'auto', padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {seasons.map(s => {
                  const active = currentSeason?.season_year === s.season_year && currentSeason?.season_quarter === s.season_quarter;
                  return (
                    <button key={`${s.season_year}-${s.season_quarter}`} onClick={() => { setCurrentSeason(s); setMenuOpen(false); }} style={{ padding: '8px 16px', borderRadius: 10, border: `1px solid ${active ? 'rgba(59,158,255,0.4)' : 'var(--border)'}`, background: active ? 'rgba(59,158,255,0.12)' : 'var(--bg-card)', color: active ? '#3B9EFF' : 'var(--text-muted)', fontFamily: 'Rajdhani, sans-serif', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
                      {s.label}
                    </button>
                  );
                })}
          ...
          {user ? (
            ...
            <button onClick={() => { setShowLogin(true); setMenuOpen(false); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '14px', borderRadius: 12, background: 'linear-gradient(135deg, #3B9EFF, #2563EB)', color: 'white', fontFamily: 'Rajdhani, sans-serif', fontWeight: 700, fontSize: 15, border: 'none', cursor: 'pointer' }}>
