...
// iRacing series logo — assets API gives correct filename (some use custom names)
function buildLogoUrl(logoFile: string): string {
  if (logoFile.startsWith('http')) return logoFile;
  if (logoFile.startsWith('img/')) return `https://images-static.iracing.com/${logoFile}`;
  return `https://images-static.iracing.com/img/logos/series/${logoFile}`;
}

function getLogoUrl(seriesId: number, logos: Record<number, string>): string {
  // Use assets API result if available (correct filename)
  if (logos[seriesId]) return logos[seriesId];
  // Fallback: try standard pattern — onError will hide if 404
...
export default function HomePage() {
  ...
  // Load assets map on mount — correct logo filenames (some custom, not seriesid_N.png)
  useEffect(() => {
      ...
      <main style={{ flex: 1, padding: isMobile ? '16px 12px 48px' : '28px 24px 48px', maxWidth: 1680, margin: '0 auto', width: '100%', paddingBottom: comparingSeries.length > 0 ? 'calc(48px + 60vh)' : isMobile ? '80px' : '48px' }}>
        ...
        {!user && (
          <div style={{
            ...
            background: isDark ? 'rgba(232,0,45,0.06)' : 'rgba(232,0,45,0.04)',
            border: '1px solid rgba(232,0,45,0.15)',
            borderRadius: 10, padding: '10px 16px', marginBottom: 20,
          }}>
            <Info size={14} color="#E8002D" style={{ flexShrink: 0 }} />
            <span style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 14, color: isDark ? '#AAAAAA' : '#666666', flex: 1 }}>
              {t.demoBannerText}
            </span>
            <button
              ...
              style={{
                ...
                color: '#E8002D', background: 'none', border: '1px solid rgba(232,0,45,0.35)',
