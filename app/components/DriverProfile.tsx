...
export default function DriverProfile({ open, onClose }: Props) {
  ...
  if (profile || loading) {
    ...
    const HelmetBadge = ({ custId, helmet, size = 140, fallbackColor = '#3B9EFF', fallbackLabel = '?' }: { custId?: number; helmet: any; size?: number; fallbackColor?: string; fallbackLabel?: string }): JSX.Element => {
      ...
      const helmetUrl = custId ? `https://images-static.iracing.com/img/helmets/${custId}.png` : null;

      if (!helmetUrl || imgError) {
        // Fallback SVG with helmet colors from API
        const c1 = helmet ? `#${helmet.color1 ?? '1E3A5F'}` : fallbackColor;
        const c2 = helmet ? `#${helmet.color2 ?? '3B9EFF'}` : fallbackColor;
        const c3 = helmet ? `#${helmet.color3 ?? 'F97316'}` : fallbackColor;
        return (
          <div style={{ width: size, height: size, borderRadius: 28, overflow: 'hidden', flexShrink: 0, boxShadow: `0 0 50px ${c1}60, 0 25px 60px rgba(0,0,0,0.6), inset 0 3px 0 rgba(255,255,255,0.2)`, border: '3px solid rgba(255,255,255,0.15)', position: 'relative' }}>
            <svg viewBox="0 0 140 140" width={size} height={size} xmlns="http://www.w3.org/2000/svg">
              <defs>
                <clipPath id="hclip"><path d="M70 8 C42 8 20 28 18 54 C16 82 30 112 50 124 C57 128 63 130 70 130 C77 130 83 128 90 124 C110 112 124 82 122 54 C120 28 98 8 70 8Z" /></clipPath>
                <radialGradient id="hshine" cx="38%" cy="28%" r="55%"><stop offset="0%" stopColor="rgba(255,255,255,0.35)" /><stop offset="60%" stopColor="rgba(255,255,255,0)" /></radialGradient>
                <radialGradient id="hvisor" cx="50%" cy="50%" r="50%"><stop offset="0%" stopColor="rgba(80,180,255,0.25)" /><stop offset="100%" stopColor="rgba(0,10,30,0.80)" /></radialGradient>
              </defs>
              <rect width="140" height="140" fill={c1} clipPath="url(#hclip)" />
              <rect x="0" y="52" width="140" height="22" fill={c2} clipPath="url(#hclip)" />
              <rect x="0" y="74" width="140" height="14" fill={c3} clipPath="url(#hclip)" />
              <ellipse cx="70" cy="72" rx="42" ry="19" fill="url(#hvisor)" clipPath="url(#hclip)" />
              <ellipse cx="56" cy="34" rx="28" ry="18" fill="url(#hshine)" clipPath="url(#hclip)" />
              <path d="M38 28 Q70 18 102 29" fill="none" stroke="rgba(255,255,255,0.55)" strokeWidth="5" strokeLinecap="round" clipPath="url(#hclip)" />
              <path d="M70 8 C42 8 20 28 18 54 C16 82 30 112 50 124 C57 128 63 130 70 130 C77 130 83 128 90 124 C110 112 124 82 122 54 C120 28 98 8 70 8Z" fill="none" stroke="rgba(255,255,255,0.22)" strokeWidth="3" />
            </svg>
          </div>
        );
      }
      ...
      return (
        <div style={{ width: size, height: size, borderRadius: 28, overflow: 'hidden', flexShrink: 0, boxShadow: '0 0 50px rgba(0,0,0,0.4), 0 25px 60px rgba(0,0,0,0.6)', border: '3px solid rgba(255,255,255,0.15)', position: 'relative', background: '#0A1221' }}>
          <img
            src={helmetUrl}
            alt="Driver helmet"
          ...
          />
          {/* Glossy overlay */}
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(255,255,255,0.25) 0%, transparent 40%, rgba(255,255,255,0.08) 100%)', pointerEvents: 'none' }} />
        </div>
      );
    };
        ...
        {loading && (
          <div style={{ maxWidth: 960, width: '100%', margin: '0 auto', padding: '40px 32px', display: 'flex', flexDirection: 'column', gap: 32 }}>
            <div style={{ padding: '48px', borderRadius: 32, border: `1px solid ${T.border}`, background: T.card, display: 'flex', alignItems: 'flex-start', gap: 36 }}>
              <div style={{ width: 140, height: 140, borderRadius: 28, flexShrink: 0, ...shimmer(isDark) }} />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 40, flexWrap: 'wrap', position: 'relative' }}>
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                    {profile.club_name && (
                      <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 13, color: T.muted, background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)', padding: '4px 10px', borderRadius: 8, border: `1px solid ${T.border}` }}>{profile.club_name}</span>
                    )}
                    {profile.member_since && (
                      <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 13, color: T.muted }}>Since {new Date(profile.member_since).getFullYear()}</span>
                    )}
                ...
                {primary && (
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontFamily: 'DM Mono, monospace', fontWeight: 900, fontSize: 52, lineHeight: 1, color: '#60A5FA', textShadow: `0 0 40px ${primaryColor}80, 0 0 80px ${primaryColor}40`, letterSpacing: '-0.02em' }}>
                      {(primary.irating ?? 0).toLocaleString()}
                    </div>
                    <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 16, color: primaryColor, fontWeight: 700, marginTop: 8, textShadow: `0 0 20px ${primaryColor}60` }}>
                      SR {(primary.safety_rating ?? 0).toFixed(2)}
                    </div>
                    <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 12, color: T.muted, marginTop: 6, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
            ...
            {profile.summary && profile.summary.total_starts > 0 && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(175px, 1fr))', gap: 16 }}>
                {[
                  ...
                  <div key={s.label}
                    style={{ padding: '22px 24px', background: T.card, borderRadius: 20, border: `1px solid ${T.border}`, transition: 'all 0.3s cubic-bezier(0.34,1.56,0.64,1)', cursor: 'default' }}
            ...
            {tab === 'stats' && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(270px, 1fr))', gap: 14 }}>
                ...
                {licenses.map(([key, lic]: [string, any]) => {
                  ...
                  return (
                    <div key={key}
                      style={{ padding: '24px', background: T.card, borderRadius: 20, border: `1px solid ${T.border}`, display: 'flex', gap: 20, alignItems: 'center', transition: 'all 0.3s cubic-bezier(0.34,1.56,0.64,1)', cursor: 'default' }}
            ...
            {tab === 'races' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                {races.map(race => {
                  ...
                  return (
                    <div key={race.subsession_id}
                      style={{ display: 'flex', alignItems: 'center', gap: 20, padding: '18px 24px', background: T.card, borderRadius: 18, border: `1px solid ${isWin ? 'rgba(234,179,8,0.35)' : T.border}`, transition: 'all 0.25s ease', cursor: 'default' }}
        ...
        <div style={{ padding: '26px 26px 20px', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.07)'}` }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '14px 16px 14px 46px', borderRadius: 16, background: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.04)', border: `1.5px solid ${isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, color: isDark ? '#F1F5F9' : '#0F172A', fontFamily: 'DM Sans, sans-serif', fontSize: 15, outline: 'none', boxSizing: 'border-box', transition: 'border-color 0.2s, box-shadow 0.2s' }}
            />
