...
import { Heart, Lock, Check, Clock, Calendar, Wrench, Flag, Trophy, GitCompare, CalendarClock } from 'lucide-react';
...
const CATEGORY_STYLE: Record<string, { gradientDark: string; gradientLight: string; accent: string; label: string }> = {
  'Sports Car':  { gradientDark: 'linear-gradient(135deg, #0E2A4A 0%, #070F1C 100%)', gradientLight: 'linear-gradient(135deg, #EBF4FF 0%, #F0F7FF 100%)', accent: '#3B9EFF', label: 'Sports Car' },
  'Formula Car': { gradientDark: 'linear-gradient(135deg, #2A0E4A 0%, #10071C 100%)', gradientLight: 'linear-gradient(135deg, #F5EEFF 0%, #F9F5FF 100%)', accent: '#A855F7', label: 'Formula'    },
  'Oval':        { gradientDark: 'linear-gradient(135deg, #2A1800 0%, #120A00 100%)', gradientLight: 'linear-gradient(135deg, #FFF4EB 0%, #FFF8F2 100%)', accent: '#F97316', label: 'Oval'        },
  'Dirt Oval':   { gradientDark: 'linear-gradient(135deg, #2A2000 0%, #121000 100%)', gradientLight: 'linear-gradient(135deg, #FEFCE8 0%, #FFFDF2 100%)', accent: '#EAB308', label: 'Dirt Oval'   },
  'Dirt Road':   { gradientDark: 'linear-gradient(135deg, #0A2A14 0%, #041209 100%)', gradientLight: 'linear-gradient(135deg, #EDFAF2 0%, #F2FCF5 100%)', accent: '#22C55E', label: 'Dirt Road'   },
  'Endurance':   { gradientDark: 'linear-gradient(135deg, #1A0A2E 0%, #0A0514 100%)', gradientLight: 'linear-gradient(135deg, #F5F0FF 0%, #FAF7FF 100%)', accent: '#E879F9', label: 'Endurance'   },
};
...
export default function SeriesCard({ series, isFavorite, isComparing, isScheduled, isEligible, onFavoriteToggle, onClick, onCompare, onSchedule }: SeriesCardProps) {
  ...
  const T = {
    ...
    cardBorder:   hovered ? accent + '55'               : isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.10)',
    cardShadow:   hovered
      ? isDark ? `0 24px 64px rgba(0,0,0,0.7), 0 0 0 1px ${accent}25` : `0 16px 40px rgba(0,0,0,0.12), 0 0 0 1px ${accent}30`
  ...
  return (
    ...
    >
      {/* ── HEADER ──────────────────────────────────────────── */}
      <div style={{ background: T.headerGrad, padding: '18px 18px 16px', position: 'relative' }}>
        <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            {series.category === 'Endurance' && duration !== '—' && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 20, background: 'rgba(232,121,249,0.15)', border: '1px solid rgba(232,121,249,0.35)', fontFamily: 'DM Mono, monospace', fontSize: 10, fontWeight: 700, color: '#E879F9', letterSpacing: '0.06em' }}>
                ⏱ {duration}
              </span>
            )}
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            {onCompare && (
              <button onClick={e => { e.stopPropagation(); onCompare(); }} title={isComparing ? t.removeFromCompare : t.addToCompare} style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: isComparing ? 'rgba(59,158,255,0.2)' : T.iconBtnBg,
        ...
        {/* Nombre */}
        <h3 style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 900, color: T.seriesName, lineHeight: 1.2, margin: '0 0 14px', letterSpacing: '-0.4px' }}>
          {series.series_name}
        </h3>
      ...
      <div style={{ padding: '14px 18px 16px', flex: 1, borderBottom: `1px solid ${T.sectionBorder}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, fontWeight: 600, color: T.labelColor, letterSpacing: '0.14em', textTransform: 'uppercase' }}>
            {t.trackRotation}
          </span>
          <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, color: accent + 'AA' }}>
            {weekCount} wks
          </span>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          {tracks.slice(0, 6).map((week, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, fontWeight: 700, color: accent, minWidth: 24, flexShrink: 0 }}>
                  W{i + 1}
                </span>
                {isActive
                ...
                {isActive && (
                  <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 9, fontWeight: 700, color: '#22C55E', background: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 4, padding: '1px 5px', flexShrink: 0 }}>
                    NOW
                  </span>
                )}
              </div>
          ...
          {tracks.length > 6 && (
            <div style={{ padding: '6px 10px', borderRadius: 8, textAlign: 'center', background: T.moreTracksBg, border: `1px solid ${T.moreTracksBorder}` }}>
              <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 11, color: T.moreTracksText }}>
      ...
      <div style={{ display: 'flex', background: T.footerBg }}>
        {[
          ...
          <div key={i} style={{ flex: 1, padding: '11px 14px', borderRight: i < 2 ? `1px solid ${T.sectionBorder}` : 'none', display: 'flex', flexDirection: 'column', gap: 3 }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontFamily: 'DM Mono, monospace', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: T.labelColor }}>
              {stat.icon} {stat.label}
            </span>
            <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 15, fontWeight: 700, color: stat.accent ? accent : T.statValue }}>
              {stat.value}
            </span>
          </div>
        ))}
