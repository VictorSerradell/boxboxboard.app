...
export default function SeriesDetailPanel({
  ...
  const [carsExtra, setCarsExtra] = useState<{
    ...
    fastest_car: string;
  }>({ sof: 0, track: "", total_drivers: 0, fastest_car: "" });
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/iracing/my-races?season_year=${series.season_year}&season_quarter=${series.season_quarter}`,
      { credentials: "include" },
    )
      .then((r) => r.json())
      .then(async (meta: any) => {
        ...
        fetch(`/api/iracing/series-stats?${statsParams}`, {
          credentials: "include",
        })
          .then((r) => r.json())
          .then((data) => {
                ...
                onClick={() => {
                  ...
                  if (tab.id === "cars" && !carsLoaded) {
                    ...
                    if (apiWeek2 != null) {
                      ...
                      for (const sched of series.schedules ?? []) {
                        if (sched.start_date) {
                          const [y, m, d] = sched.start_date
                            .split("-")
                            .map(Number);
                          if (new Date(y, m - 1, d) <= now2)
                    ...
                    fetch(
                      ...
                      .then(async (meta: any) => {
                        ...
                        if (meta.type === "inline") {
                          ...
                          const fetches = await Promise.allSettled(
                            meta.chunks.map((file: string) =>
                              fetch(meta.base_url + file).then((r) =>
                                r.ok ? r.json() : [],
                              ),
                            ),
                          );
                        ...
                        return fetch(`/api/iracing/series-cars?${carsParams}`, {
                          credentials: "include",
                        }).then((r) => r.json());
                      })
                      .then((data) => {
                    ...
                    >
                      {weekCars.length > 0 ? (
                        <>
                          ...
                          >
                            Los tiempos de vuelta solo están disponibles para
                            series en las que hayas participado
                          </div>
                        </>
                      ) : (
                        <div
                  ...
                  >
                    <SectionTitle>
                      <Car
                        ...
                        style={{ display: "inline", marginRight: 5 }}
                      />
                      Top 10 tiempos · {carsExtra.track}
                    </SectionTitle>
                    {carsExtra.sof > 0 && (
                      <span
                        style={{
