...
import {
  X, Heart, Lock, Check, Clock, Calendar, Wrench, Flag, Trophy,
  Car, MapPin, BarChart2, Users, ChevronRight, Zap, Weight,
} from 'lucide-react';
...
const DEMO_CARS: Record<number, { name: string; owned: boolean; free: boolean }[]> = {
  ...
  302: [
    { name: 'Porsche 911 GT3 Cup (992)',       owned: true,  free: false },
  ],
  303: [
    { name: 'Dallara F3',                     owned: false, free: true  },
  ],
  304: [
  ...
  308: [
    { name: 'Dallara IR18',                   owned: false, free: false },
  ],
  310: [
    { name: 'Super Formula SF23',             owned: false, free: false },
  ],
  312: [
    { name: 'ORECA LMP2 07',                  owned: true,  free: false },
  ],
};
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <p style={{
      ...
      color: '#334155', letterSpacing: '0.14em', textTransform: 'uppercase',
      margin: '0 0 10px',
    }}>
      {children}
    </p>
...
function StatBox({ icon, label, value, accent }: { icon: React.ReactNode; label: string; value: string; accent?: string }) {
  return (
    <div style={{
      ...
      <span style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#334155', fontSize: 10, fontFamily: 'DM Mono, monospace', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
        {icon} {label}
      </span>
      <span style={{ fontSize: 18, fontWeight: 800, fontFamily: 'Syne, sans-serif', color: accent ?? 'rgba(255,255,255,0.9)' }}>
        {value}
      </span>
    </div>
  );
    ...
    <>
      ...
      <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <h2 style={{
                ...
                color: 'white', margin: '0 0 12px', letterSpacing: '-0.4px', lineHeight: 1.2,
              }}>
                {series.series_name}
              </h2>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                <span style={{
                  ...
                  background: licConfig.color + '22', border: `1px solid ${licConfig.color}50`,
                ...
                <span style={{
                  ...
                  padding: '4px 11px', borderRadius: 20, fontSize: 12, fontWeight: 700,
                  fontFamily: 'DM Mono, monospace',
                  background: series.fixed_setup ? 'rgba(59,158,255,0.14)' : 'rgba(34,197,94,0.14)',
                ...
                {series.official && (
                  <span style={{
                    ...
                    padding: '4px 11px', borderRadius: 20, fontSize: 12, fontWeight: 700,
                    fontFamily: 'DM Mono, monospace',
                    background: 'rgba(234,179,8,0.14)', border: '1px solid rgba(234,179,8,0.38)',
                    color: '#EAB308',
                  }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0 }}>
              ...
              <button onClick={handleFav} style={{
                width: 36, height: 36, borderRadius: 10, cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: localFav ? 'rgba(239,68,68,0.18)' : 'rgba(255,255,255,0.07)',
              ...
              <div style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: series.isOwned ? 'rgba(34,197,94,0.15)' : 'rgba(255,255,255,0.07)',
        ...
        <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          {/* ESTADÍSTICAS */}
          <section>
            <SectionTitle><BarChart2 size={10} style={{ display: 'inline', marginRight: 5 }} />Stats</SectionTitle>
            <div style={{ display: 'flex', gap: 8 }}>
              <StatBox icon={<Zap size={11} />}    label="Avg SOF"    value={stats.avg_sof.toLocaleString()} accent={accent} />
              <StatBox icon={<Users size={11} />}  label="Avg Drivers" value={String(stats.avg_drivers)} />
              <StatBox icon={<BarChart2 size={11} />} label="Splits"  value={String(stats.splits)} />
              <StatBox icon={<Clock size={11} />}  label="Race Time"  value={duration} />
            </div>
          </section>

          {/* COCHES PERMITIDOS */}
          <section>
            <SectionTitle><Car size={10} style={{ display: 'inline', marginRight: 5 }} />Allowed Cars</SectionTitle>
                ...
                <div key={i} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      background: car.owned ? 'rgba(34,197,94,0.12)' : 'rgba(255,255,255,0.05)',
                    ...
                    <span style={{ fontSize: 13, fontWeight: 500, color: car.owned ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.45)' }}>
                      {car.name}
                    </span>
                  </div>
                  {car.free && (
          ...
          {/* HORARIOS */}
          <section>
            <SectionTitle><Clock size={10} style={{ display: 'inline', marginRight: 5 }} />Race Schedule</SectionTitle>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {times.map((t, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: 13, fontWeight: 500, color: 'rgba(255,255,255,0.8)', fontFamily: 'DM Mono, monospace' }}>
                    {t}
                  </span>
                </div>
              ))}
          ...
          {/* CALENDARIO COMPLETO */}
          <section>
            <SectionTitle><Calendar size={10} style={{ display: 'inline', marginRight: 5 }} />Full Track Calendar</SectionTitle>
              ...
              {series.schedules.map((week, i) => (
                <div key={i} style={{
                  ...
                  <span style={{
                    ...
                    color: accent, minWidth: 28, flexShrink: 0,
                  }}>
                    W{i + 1}
                  </span>

                  <Flag size={12} strokeWidth={1.8} color="rgba(255,255,255,0.2)" style={{ flexShrink: 0 }} />
          ...
          <section>
            ...
            <div style={{
              ...
              {[
                { label: 'Season',       value: `S${series.season_quarter} ${series.season_year}` },
                { label: 'Min License',  value: series.allowed_licenses?.[0]?.group_name ?? '—' },
                { label: 'Team Driving', value: series.driver_changes ? 'Yes' : 'No' },
                { label: 'Multiclass',   value: series.multiclass ? 'Yes' : 'No' },
              ].map(item => (
                <div key={item.label} style={{
                  padding: '10px 14px', borderRadius: 10,
                  background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)',
                }}>
                  <div style={{ fontSize: 10, fontFamily: 'DM Mono, monospace', color: '#334155', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>
                    {item.label}
                  </div>
